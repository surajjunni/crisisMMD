...
if __name__ == "__main__":
    ...
    #options.train_data="task_informative_text_img_agreed_lab_train.tsv"
    #options.val_data="task_informative_text_img_agreed_lab_dev.tsv"
    #options.test_data="task_informative_text_img_agreed_lab_test.tsv"
    ...
    #base_name = os.path.splitext(base_name)[0]
    log_file = "image_Case1_log_v2.txt"
    ...
    if (options.w2v_checkpoint and file_exist(options.w2v_checkpoint)):
        ...
        word_vec_model_file = "crisisNLP_word2vec_model/crisisNLP_word_vector.bin"
    ...
    print(cnn)
    # R, C = train_x.shape
    text_network = Dense(1000, activation='relu',name=tensorflow.compat.v1.get_default_graph().unique_name("dense"))(cnn)
    #print(text_network)
    text_network = BatchNormalization(name="bn1")(text_network)
    #print(text_network)
    text_network = Dense(2000,use_bias=False,name=tensorflow.compat.v1.get_default_graph().unique_name("dense"))(text_network)  
    ######## Image text_network ########
    last_layer_output, vgg16 = vgg_model()
 
    #print(vgg16)
    last_layer_output = Dense(1000, activation='relu',name=tensorflow.compat.v1.get_default_graph().unique_name("dense"))(last_layer_output)
    #print(last_layer_output)
    last_layer_output = BatchNormalization(name="bn2")(last_layer_output)
    last_layer_output = Dense(2000,use_bias=False,name=tensorflow.compat.v1.get_default_graph().unique_name("dense"))(last_layer_output)
    #print(last_layer_output)
    ######## Merge image and text networks ########

    #merged_network = keras.layers.Add()([x.output,y.output])
    #merged_network = concatenate([text_network, last_layer_output], name='Concatenate')
    #print(merged_network)
    #x1 = Dense(2000, use_bias=False)(x.output)
    #x2 = Dense(2000,use_bias=False)(y.output)
    merged_network = tensorflow.keras.layers.Add()([text_network, last_layer_output])
    merged_network = BatchNormalization(name="bn3")(merged_network)
    merged_network = Dropout(0.4)(merged_network)
    merged_network = Dense(500, activation='relu',name=tensorflow.compat.v1.get_default_graph().unique_name("dense"))(merged_network)
    merged_network = Dropout(0.2)(merged_network)
    merged_network = Dense(100, activation='relu',name=tensorflow.compat.v1.get_default_graph().unique_name("dense"))(merged_network)
    merged_network = Dropout(0.02)(merged_network)
    out = Dense(nb_classes, activation='softmax',name=tensorflow.compat.v1.get_default_graph().unique_name("dense"))(merged_network)
    #print(out)
    #print(vgg16.input, inputs)
    #concatted = tensorflow.keras.layers.Concatenate()([vgg16.input, inputs])
    if(os.path.exists(best_model_path)):
       print("from checkpoint")
       model=load_model(best_model_path)
       nb_epoch=2
    else:
       model = Model([vgg16.input, inputs], out)


    lr = 0.00001
    print("lr= "+str(lr)+", beta_1=0.9, beta_2=0.999, amsgrad=False")
    adam = tensorflow.keras.optimizers.Adam(lr=lr, beta_1=0.9, beta_2=0.999, amsgrad=False)
    ...
    history = model.fit_generator(generator=train_data_generator, epochs=nb_epoch, validation_data=val_data_generator, verbose=1, callbacks=callbacks_list)
    ...
    AUC, accu, P, R, F1, report = performance.performance_measure_cnn(dev_y[:len(dev_prob)], dev_prob, train_le)
